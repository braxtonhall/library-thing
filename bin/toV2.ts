...
const toV2 = (v3) =>
	Object.entries(v3).reduce((v2, [key, value]): Record<string, unknown> => {
...
const retainKey =
	(callback: ({value, manifest}) => unknown): Transformer =>
	({key, value, manifest}) => ({key, value: callback({value, manifest})});
