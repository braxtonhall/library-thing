...
const createSection = () => {
	...
	section.className = "simpleSection worklist";
	return section;
};
...
const createCurrentTagsSection = (onEdit: () => void) => {
	const section = createSection();
...
const appendUI = (container: Element, onEdit: () => void, onSave: () => void) => {
	const header = createHeader("Tags");
	const currentTagsSection = createCurrentTagsSection(onEdit);
